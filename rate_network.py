...
def simulate_seq(t : np.ndarray, n_e : int, n_i : int, inp : np.ndarray, transfer_e, transfer_i, plasticity_coefs : np.ndarray, w : np.ndarray, tau_e=5e-3, tau_i=5e-3, tau_a=10e-3, dt=1e-6, g=1, w_u=1):    
    ...
    inp_r = np.zeros((len(t), n_e + n_i))
    inp_r[:, 0] = inp

    for i in range(0, len(t) - 1):
        v[i+1, :] = w_u * inp_r[i, :] + np.dot(w_copy, r[i, :].T)
